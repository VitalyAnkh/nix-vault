...
> All the operations in this section should be performed in your private secrets repository (the one
> referenced by `mysecrets` in `flake.nix`, e.g. `vr-nix-secrets`).
...
let
  ...
  # A key for recovery purpose, generated by `ssh-keygen -t ed25519 -a 256 -C "vitalyr@agenix-recovery"` with a strong passphrase
  # and keeped it offline in a safe place.
  recovery_key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIHnIGH+653Oe+GQaA8zjjj7HWMWp7bWXed4q5KqY4nqG vitalyr@agenix-recovery";
  systems = [
    idol_ai
    fern
...
```nix
{
  inputs = {
    ...
    mysecrets = { url = "github:VitalyAnkh/vr-nix-secrets"; flake = false; };
  };
